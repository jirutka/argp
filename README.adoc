...
If you don‘t use subcommands and want to reduce the binary size by ~4{nbsp}kiB, you can build argp without the `subcommands` feature:
...
=== Expand the derive macro in `examples/subcommands.rs`

See link:argp/examples/subcommands.rs[] for the example struct we wish to expand.
